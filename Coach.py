...
class Coach():
    ...
    def learn(self):
        ...
        for i in range(1, self.args.numIters+1):
            ...
            if not self.skipFirstSelfPlay or i>1:
                ...
                for eps_i in range(1, int(self.args.numEps/4) + 1):
                    ...
                    for epsResult in epsResults:
                        iterationTrainExamples += epsResult
            ...
            pmcts = MCTS(self.game, pnet, self.args, verbose=True)

            nnet.train(trainExamples)
            nmcts = MCTS(self.game, nnet, self.args, verbose=True)
            ...
            if pwins+nwins == 0 or float(nwins)/(pwins+nwins) < self.args.updateThreshold:
                ...
                nnet.save_checkpoint(folder=self.args.checkpoint, filename=self.getCheckpointFile(i))
                nnet.save_checkpoint(folder=self.args.checkpoint, filename='best')
                self.args.checkpoint_filename = 'best'    
